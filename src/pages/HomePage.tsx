...
import { Bookmark, Share2, BookOpen, Loader2 } from "lucide-react";
...
export default function HomePage() {
  ...
  if (!scripture) {
    return (
      <div className="px-5 pt-safe pt-6 text-center">
  ...
  return (
    <div className="px-5 pt-safe">
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="pt-6 pb-2">
        <p className="text-xs font-body text-muted-foreground tracking-widest uppercase">{greeting()}</p>
        <h1 className="font-display text-2xl font-semibold mt-1 text-foreground">Today's Word</h1>
      </motion.div>
      ...
      >
        <div className="flex items-center gap-2 mb-5">
          ...
          <span className="px-3 py-1 rounded-full bg-muted text-xs font-body text-muted-foreground">
        ...
        <p className="mt-4 font-body text-sm font-medium text-gold-dark">— {scripture.reference}</p>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-xs font-body font-medium text-muted-foreground uppercase tracking-widest mb-1.5">Reflection</p>
            <p className="text-sm font-body leading-relaxed text-foreground/80">{scripture.reflection}</p>
          </div>
        </div>
        ...
        <div className="flex gap-3 mt-6">
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl font-body text-sm font-medium transition-colors ${
              saved ? "bg-gold/10 text-gold-dark" : "bg-scripture text-foreground"
            }`}
          >
      ...
      <motion.div
        ...
        className="mt-8 mb-6"
      >
        <h2 className="font-display text-lg font-semibold text-foreground mb-3">Explore Themes</h2>
        <div className="flex gap-2 flex-wrap">
          {["Leadership", "Courage", "Faith", "Wisdom", "Perseverance", "Purpose"].map((t) => (
            <a
              key={t}
